...
sampleplayer.CastPlayer.prototype.preloadVideo_ = function(mediaInformation) {
  ...
  var host = new cast.player.api.Host({
   ...
   var jsonO = [];
	  ...
	  var item = {};
	  item ["userId"] = "12345";
	  item ["sessionId"] = "1234567";
	  item ["merchant"] = "afrostream";
	  jsonObj.push(item);
	  
	  	  var item2 = {};
	    item2 ["x-dt-custom-data"] = JSON.stringify(jsonObj);
...
//  host.licenseCustomData =window.btoa(customData);
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
  ...
  if (!protocolFunc) {
    ...
    if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
	  ...
	  var item = {};
	  item ["userId"] = "12345";
	  item ["sessionId"] = "1234567chromecast";
	  item ["merchant"] = "afrostream";
	  jsonObj.push(item);
		...
		//host.protectionSystem=cast.player.api.ContentProtection.PLAYREADY;
		
		//host.licenseUrl = "https://lic.staging.drmtoday.com/license-proxy-headerauth/drmtoday/RightsManager.asmx";
		//host.licenseUrl = "https://lic.staging.drmtoday.com/license-proxy-widevine/cenc/?specConform=false";
		
		//host.licenseCustomData ="eyJ1c2VySWQiOiIxMjM0NSIsInNlc3Npb25JZCI6IjEyMzQ1NjdjaHJvbWVjYXN0IiwibWVyY2hhbnQiOiJhZnJvc3RyZWFtIn0=";// window.btoa(customData);
			...
			host.updateLicenseRequestInfo = function(requestInfo) {
				...
				//requestInfo.skipRequest =true;
					...
					requestInfo.protectionSystem=cast.player.api.ContentProtection.PLAYREADY;
				 var jsonO = [];
				  var jsonObj = [];
				  var item = {};
				  item ["userId"] = "12345";
				  ...
				  jsonObj.push(item);
					  ...
					  var item2 = {};
					item2 ["x-dt-custom-data"] = window.btoa(JSON.stringify(jsonObj));
				  
					jsonO.push(item2);
			var tr=JSON.stringify(jsonO);
			console.log ("updateLicenseRequestInfo "+tr);
			
			
			//requestInfo.headers = {};
        //requestInfo.headers['content-type'] = 'application/json;charset=utf-8';
	
			requestInfo.headers["Content-Type"]="text/xml"; 										 
		   requestInfo.headers["x-dt-custom-data"] ="eyJ1c2VySWQiOiIxMjM0NSIsInNlc3Npb25JZCI6IjEyMzQ1NjdjaHJvbWVjYXN0IiwibWVyY2hhbnQiOiJhZnJvc3RyZWFtIn0=";//window.btoa(JSON.stringify(jsonObj));
		};
		
		
		
		host.processLicense = function(responseArray) {

			  var response = String.fromCharCode.apply(null, responseArray); 
			  console.log ("licence reponse : "+ response);
	  ...
	  host.updateManifestRequestInfo = function(requestInfo) {
		...
		host.updateLicenseRequestInfo = function(requestInfo) {
			...
			console.log ("updateLicenseRequestInfo "+requestInfo);
		  //requestInfo.withCredentials = true;
		};
...
*/
      ...
      this.player_ = new cast.player.api.Player(host);
	   ...
	   var initStart = 0;//info.data['media']['currentTime'] || 0;
...
sampleplayer.CastPlayer.prototype.processInBandTracks_ =
    function(activeTrackIds) {
		...
		console.log ("track :"+activeTrackIds)
		...
		var protocol = this.player_.getStreamingProtocol();
		 ...
		 //console.log(streamInfo.mimeType +"   true");
			...
			} else if (!isActive && wasActive && !isvideo) {
					 var streamInfo = protocol.getStreamInfo(i);
					   ...
					   console.log("stream role "+ streamInfo.role);
					 
					 if (streamInfo.name.indexOf('video')===-1)
					 {
						 
						  console.log("stream info false "+streamInfo.name);
							  protocol.enableStream(i, false);
								if(streamInfo.name.indexOf('text')===-1) this.player_.reload();
					 }
...
var trackId =0;
  ...
  console.log ("current :"+currentLanguage);
   ...
   streamInfo = protocol.getStreamInfo(trackId);
   if (streamInfo.mimeType.indexOf('audio') === 0) {
...
sampleplayer.CastPlayer.prototype.onEditTracksInfo_ = function(event) {
  ...
  console.log ("edit track event :"+ type);
